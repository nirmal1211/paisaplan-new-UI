...
type DashboardSection = 'overview' | 'vehicle' | 'drivers' | 'coverage' | 'claims' | 'documents' | 'calculator' | 'challans';
...
const VehicleInsuranceDetailsPage: React.FC = () => {
  ...
  const challanSummary = {
    ...
    totalFineAmount: mockChallans.reduce((sum, challan) => sum + challan.fineAmount, 0),
    paidAmount: mockChallans.filter(c => c.paymentStatus === 'paid').reduce((sum, challan) => sum + challan.fineAmount, 0),
    pendingAmount: mockChallans.filter(c => c.paymentStatus === 'pending').reduce((sum, challan) => sum + challan.fineAmount, 0)
  };
