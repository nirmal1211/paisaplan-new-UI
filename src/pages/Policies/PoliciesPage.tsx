  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <div>
          <h1 className="text-4xl font-extrabold text-gray-900 font-poppins tracking-tight">
            Policies
          </h1>
          <p className="text-gray-600 font-roboto mt-2 text-lg">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-10 border border-slate-100">
        ...
        {!selectedType && (
          <div>
            <h2 className="text-2xl font-semibold text-gray-900 font-poppins mb-8">
              Select Policy Type
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
              {policyTypes.map((type) => (
                <button
                  key={type}
                  className={`group relative rounded-xl p-8 border shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-200 hover:shadow-lg hover:-translate-y-1 ${
                    policyTypeStyles[type] ||
                    "bg-gray-50 border-gray-200 text-gray-800"
                  }`}
                  onClick={() =>
                    navigate(`/my-policy/${encodeURIComponent(type)}`)
                  }
                >
                  <div className="flex items-center mb-2">
                    <Layers className="h-8 w-8 mr-4 opacity-80" />
                    <span className="text-xl font-bold font-poppins tracking-wide">
                      {type}
                    </span>
                  </div>
                  <span className="font-roboto text-base font-medium opacity-80">
                    {policies.filter((p) => p.type === type).length} Policy
                    {policies.filter((p) => p.type === type).length > 1
                      ? "ies"
                      : "y"}
                  </span>
                </button>
          ...
          <div>
            <div className="flex items-center justify-between mb-10">
              <div className="flex items-center space-x-4">
                ...
                <h3 className="text-2xl font-extrabold text-gray-900 font-poppins tracking-tight">
                  {selectedType} Policies
                </h3>
              </div>
              <button className="flex items-center gap-2 px-5 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold shadow-md transition-all text-base">
                <Plus className="h-5 w-5" />
                Add Policy
              </button>
            </div>
            {filteredPolicies.length === 0 ? (
              <div className="text-gray-500 font-roboto">
                No policies found for this type.
              </div>
            ) : (
              <div className="grid gap-8">
                {filteredPolicies.map((policy) => (
                  <div
                    key={policy.id}
                    className="bg-white border border-slate-100 rounded-xl p-8 hover:shadow-lg transition-shadow flex flex-col md:flex-row md:items-center md:justify-between shadow-md"
                  >
                    <div className="flex items-center mb-4 md:mb-0">
                      <div
                        className={`w-8 h-8 rounded-xl mr-6 shadow ${policy.color}`}
                      />
                      <div>
                        <h3 className="text-xl font-bold text-gray-900 font-poppins tracking-wide">
                          {policy.type}
                        </h3>
                        <p className="text-base text-gray-500 font-roboto">
                          {policy.number}
                        </p>
                      </div>
                    </div>
                    ...
                    <span
                      className={`ml-0 md:ml-6 mt-4 md:mt-0 px-5 py-2 rounded-full text-base font-bold shadow-md ${
