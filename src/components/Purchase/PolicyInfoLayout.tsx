...
import { 
  Heart, Shield, Bike, Car, Plus, Trash2, Phone, Check, ArrowLeft, 
  Star, Users, Award, Clock, FileText, ChevronRight, Info
} from 'lucide-react';
...
const PolicyInfoLayout: React.FC = () => {
  ...
  const policyInfo = {
    overview: {
      ...
      description: 'Comprehensive coverage designed to protect what matters most to you.',
      keyFeatures: [
    ...
    coverage: {
      ...
      sections: [
        {
          name: 'Basic Coverage',
          items: ['Primary protection as per policy terms', 'Standard benefits included', 'Minimum coverage requirements']
        },
        {
          name: 'Enhanced Coverage',
          items: ['Additional protection options', 'Premium add-on benefits', 'Extended coverage limits']
        },
        {
          name: 'Exclusions',
          items: ['Pre-existing conditions (waiting period applies)', 'War and nuclear risks', 'Self-inflicted injuries']
        }
      ]
    },
    process: {
      title: 'How It Works',
      steps: [
        { step: 1, title: 'Fill Application', description: 'Complete your policy application with accurate information' },
        { step: 2, title: 'Choose Provider', description: 'Select from our network of trusted insurance providers' },
        { step: 3, title: 'Customize Coverage', description: 'Add optional coverage and customize your policy' },
        { step: 4, title: 'Make Payment', description: 'Secure payment processing with instant policy issuance' }
      ]
    }
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-10 gap-8">
            ...
            <div className="lg:col-span-7">
              ...
              <div className="text-center mb-8">
                ...
                <h1 className="text-4xl font-bold font-poppins mb-2" style={{ color: 'var(--color-foreground)' }}>
              ...
              {/* Navigation Tabs */}
              <div className="flex space-x-1 mb-8 p-1 rounded-xl" style={{ backgroundColor: 'var(--color-secondary)' }}>
                {['overview', 'coverage', 'process'].map((section) => (
                  <button
                    key={section}
                    onClick={() => setActiveSection(section)}
                    className={`flex-1 px-4 py-3 rounded-lg font-medium font-roboto transition-all duration-200 ${
                      activeSection === section ? 'shadow-sm' : 'hover:opacity-80'
                    }`}
                    style={{
                      backgroundColor: activeSection === section ? 'var(--color-card)' : 'transparent',
                      color: activeSection === section ? 'var(--color-primary)' : 'var(--color-muted)'
                    }}
                  >
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="rounded-xl shadow-lg p-8" style={{ backgroundColor: 'var(--color-card)' }}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                          <h3 className="text-lg font-semibold font-poppins mb-4" style={{ color: 'var(--color-foreground)' }}>
                            Key Features
                          </h3>
                          <div className="space-y-3">
                            {policyInfo.overview.keyFeatures.map((feature, index) => (
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            {policyInfo.overview.benefits.map((benefit, index) => (
                              <div key={index} className="flex items-start space-x-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="text-center p-6 rounded-xl" style={{ backgroundColor: 'var(--color-card)' }}>
                      ...
                      <div className="text-center p-6 rounded-xl" style={{ backgroundColor: 'var(--color-card)' }}>
                        <Award className="h-8 w-8 mx-auto mb-3" style={{ color: 'var(--color-primary)' }} />
                        <div className="text-2xl font-bold font-poppins" style={{ color: 'var(--color-foreground)' }}>
                          95%
                        </div>
                        <div className="text-sm font-roboto" style={{ color: 'var(--color-muted)' }}>
                      ...
                      <div className="text-center p-6 rounded-xl" style={{ backgroundColor: 'var(--color-card)' }}>
                        <Clock className="h-8 w-8 mx-auto mb-3" style={{ color: 'var(--color-primary)' }} />
                        <div className="text-2xl font-bold font-poppins" style={{ color: 'var(--color-foreground)' }}>
                          24/7
                        </div>
                        <div className="text-sm font-roboto" style={{ color: 'var(--color-muted)' }}>
                          ...
                          <div className="space-y-2">
                            {section.items.map((item, itemIndex) => (
                              <div key={itemIndex} className="flex items-start space-x-3">
                                <ChevronRight className="h-4 w-4 mt-0.5 flex-shrink-0" style={{ color: 'var(--color-primary)' }} />
                                <span className="font-roboto" style={{ color: 'var(--color-foreground)' }}>
                                  {item}
                                </span>
                              </div>
            ...
            <div className="lg:col-span-3">
              <div className="sticky top-8">
                <form onSubmit={handleSubmit} className="rounded-xl shadow-2xl p-6 border" style={{ 
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <button
                          ...
                          className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1 rounded text-xs font-medium transition-colors"
                          style={{
                        ...
                        >
                          {state.formData.mobileVerified ? (
                            <><Check className="h-3 w-3 inline mr-1" />Verified</>
                          ) : (
                            <>Verify</>
                          )}
                        </button>
                      ...
                      {errors.mobile && (
                        <p className="text-xs font-roboto" style={{ color: 'var(--color-danger)' }}>
                          {errors.mobile}
                        </p>
                      )}
                    </div>
                    ...
                    {state.formData.mobileVerified && (
                      <div className="space-y-4 animate-slide-down">
                        {currentPolicyType.baseFields.slice(0, 2).map(renderField)}
                      </div>
                    )}

                    {/* Dependents for Health Insurance */}
                    {policyType === 'health-insurance' && state.formData.mobileVerified && (
                      <div className="space-y-4 animate-slide-down">
                        ...
                        {state.formData.dependents?.map((dependent, index) => (
                          <div key={dependent.id} className="border rounded-lg p-3 space-y-3" style={{ borderColor: 'var(--color-border)' }}>
                            <div className="flex items-center justify-between">
                              <span className="text-xs font-medium font-roboto" style={{ color: 'var(--color-foreground)' }}>
                                Dependent {index + 1}
                              </span>
                              <button
                                type="button"
                            ...
                            <input
                              ...
                              placeholder="Name"
                              className="w-full px-3 py-2 border rounded text-sm font-roboto focus:outline-none focus:ring-1 transition-all"
                              style={{
                                borderColor: 'var(--color-border)',
                                backgroundColor: 'var(--color-background)',
                                color: 'var(--color-foreground)',
                  ...
                  <button
                    type="submit"
                    className="w-full mt-6 py-3 px-6 rounded-lg font-bold font-roboto text-white transition-all duration-200 hover:opacity-90"
