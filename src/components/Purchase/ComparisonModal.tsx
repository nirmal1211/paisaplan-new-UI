...
import { X, Star, Check, ArrowRight, Users, TrendingUp, Shield, Phone, Info } from 'lucide-react';
...
const ComparisonModal: React.FC<ComparisonModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      ...
      >
        {/* Compact Header */}
        <div className="p-4 border-b sticky top-0 z-10 backdrop-blur-md" style={{ 
        ...
        <div className="overflow-y-auto max-h-[calc(80vh-60px)] custom-scrollbar">
          <div className="p-4">
              ...
              <table className="w-full">
                <thead>
                  <tr>
                    ...
                    {compareProviders.map(provider => (
                      <th key={provider.id} className="w-1/4 px-3 py-4 text-center">
                        <div className="flex flex-col items-center">
                          ...
                          />
                          <h3 className="font-bold font-poppins text-base" style={{ color: 'var(--color-foreground)' }}>
                            {provider.name}
                          </h3>
                ...
                <tbody>
                  ...
                  <tr className="border-t" style={{ borderColor: 'var(--color-border)' }}>
                    <td className="px-3 py-4 font-medium" style={{ color: 'var(--color-foreground)' }}>
                      <div className="flex items-center space-x-2">
                        <span>Premium</span>
                        <div className="group relative">
                          <Info className="h-4 w-4 cursor-help" style={{ color: 'var(--color-muted)' }} />
                          <div className="absolute left-0 bottom-full mb-2 w-48 p-2 rounded-lg text-xs opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20" style={{ backgroundColor: 'var(--color-foreground)', color: 'var(--color-background)' }}>
                  ...
                  <tr className="border-t" style={{ borderColor: 'var(--color-border)' }}>
                    ...
                    {compareProviders.map(provider => (
                      <td key={provider.id} className="px-3 py-4 text-center">
                        ...
                        <p className="text-xs" style={{ color: 'var(--color-muted)' }}>hospitals</p>
                      </td>
                    ))}
                  </tr>
            ...
            {/* Action Buttons */}
            <div className="mt-6 pt-4 border-t flex flex-wrap justify-center gap-4" style={{ borderColor: 'var(--color-border)' }}>
              {compareProviders.map(provider => (
                <button
                  key={provider.id}
                  onClick={() => {
